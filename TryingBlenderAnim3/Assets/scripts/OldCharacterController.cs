...
public class OldCharacterController : MonoBehaviour
{
    ...
    //{
    //	if (!firstTimeAdjust && adjustCounter == 0) return;
    //	if(firstTimeAdjust)  {
    //		if (dif > 180f)
    //			dif = dif - 360f;
    //		else if (dif < -180f)
    ...
    {
        AnimatorStateInfo anim = myAnimator.GetCurrentAnimatorStateInfo(0);
        return (!anim.IsTag("Jumps") && myAnimator.GetFloat("VSpeed") == 0 && myAnimator.GetFloat("HorizSpeed") == 0 &&
            myAnimator.GetBool("shouldFrontFlip") == false && myAnimator.GetBool("Jumping") == false);
    }

    public bool moving()
    {
        return Mathf.Abs(myAnimator.GetFloat("HorizSpeed")) > 0f
            || Mathf.Abs(myAnimator.GetFloat("VSpeed")) > 0f;
    }
